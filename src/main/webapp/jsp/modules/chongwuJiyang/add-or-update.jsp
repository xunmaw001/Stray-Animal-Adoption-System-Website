            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑宠物寄养</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>宠物名称</label>
                                        <input id="chongwuJiyangName" name="chongwuJiyangName" class="form-control"
                                               placeholder="宠物名称">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>宠物类别</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>年龄</label>
                                        <input id="chongwuJiyangAge" name="chongwuJiyangAge" class="form-control"
                                               placeholder="年龄">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>花色</label>
                                        <input id="chongwuJiyangHuase" name="chongwuJiyangHuase" class="form-control"
                                               placeholder="花色">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>健康情况</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>所在地</label>
                                        <input id="chongwuJiyangSuozaidi" name="chongwuJiyangSuozaidi" class="form-control"
                                               placeholder="所在地">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>宠物照片</label>
                                        <img id="chongwuJiyangPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="chongwuJiyangPhotoupload"
                                               class="form-control-file">
                                        <input name="chongwuJiyangPhoto" id="chongwuJiyangPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>宠物详情</label>
                                        <input id="chongwuJiyangContentupload" name="file" type="file">
                                        <script id="chongwuJiyangContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var chongwuJiyangContentUe = UE.getEditor('chongwuJiyangContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="chongwuJiyangContent" id="chongwuJiyangContent-input">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>预约时间</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>寄养天数</label>
                                        <input id="chongwuJiyangNumber" name="chongwuJiyangNumber" class="form-control"
                                               onchange="chongwuJiyangNumberChickValue(this)"  placeholder="寄养天数">
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label>是否接收</label>
                                        <select id="chongwuJiyangTypesSelect" name="chongwuJiyangTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#chongwuJiyangPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('chongwuJiyangPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('chongwuJiyangPhoto-input').setAttribute('value',photoUrl);
            }
        });


        $('#chongwuJiyangContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('chongwuJiyangContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
        ...
        if (validform() == true && compare() == true) {
           ...
           if($("#yonghuId") !=null){
            ...
            httpJson("chongwuJiyang/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addchongwuJiyang');
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationChongwujiyangtypesSelect(){
            var chongwuJiyangTypesSelect = document.getElementById('chongwuJiyangTypesSelect');
            if(chongwuJiyangTypesSelect != null && chongwuJiyangTypesOptions != null  && chongwuJiyangTypesOptions.length > 0 ){
                for (var i = 0; i < chongwuJiyangTypesOptions.length; i++) {
                    chongwuJiyangTypesSelect.add(new Option(chongwuJiyangTypesOptions[i].indexName,chongwuJiyangTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(chongwuJiyangTypesSelect != null && chongwuJiyangTypesOptions != null  && chongwuJiyangTypesOptions.length > 0 ) {
            for (var i = 0; i < chongwuJiyangTypesOptions.length; i++) {
                if (chongwuJiyangTypesOptions[i].codeIndex == ruleForm.chongwuJiyangTypes) {//下拉框value对比,如果一致就赋值汉字
                        chongwuJiyangTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.chongwuJiyangContent != null && ruleForm.chongwuJiyangContent != 'null' && ruleForm.chongwuJiyangContent != '' && $("#chongwuJiyangContentupload").length>0) {

            var chongwuJiyangContentUeditor = UE.getEditor('chongwuJiyangContentEditor');
            chongwuJiyangContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.chongwuJiyangContent != null){
                    mes = ''+ ruleForm.chongwuJiyangContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                chongwuJiyangContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#chongwuJiyangContentupload").length>0) {
            var chongwuJiyangContentEditor = UE.getEditor('chongwuJiyangContentEditor');
            if (chongwuJiyangContentEditor.hasContents()) {
                $('#chongwuJiyangContent-input').attr('value', chongwuJiyangContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function chongwuJiyangNumberChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yonghuId: "required",
                chongwuJiyangName: "required",
                chongwuTypes: "required",
                chongwuSexTypes: "required",
                chongwuJiyangAge: "required",
                chongwuJiyangHuase: "required",
                chongwuJiankangTypes: "required",
                chongwuJiyangSuozaidi: "required",
                chongwuJiyangPhoto: "required",
                chongwuJiyangContent: "required",
                yuyueTime: "required",
                chongwuJiyangNumber: "required",
                chongwuJiyangTypes: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                chongwuJiyangName: "宠物名称不能为空",
                chongwuTypes: "宠物类别不能为空",
                chongwuSexTypes: "性别不能为空",
                chongwuJiyangAge: "年龄不能为空",
                chongwuJiyangHuase: "花色不能为空",
                chongwuJiankangTypes: "健康情况不能为空",
                chongwuJiyangSuozaidi: "所在地不能为空",
                chongwuJiyangPhoto: "宠物照片不能为空",
                chongwuJiyangContent: "宠物详情不能为空",
                yuyueTime: "预约时间不能为空",
                chongwuJiyangNumber: "寄养天数不能为空",
                chongwuJiyangTypes: "是否接收不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchongwuJiyang = window.sessionStorage.getItem("addchongwuJiyang");
        if (addchongwuJiyang != null && addchongwuJiyang != "" && addchongwuJiyang != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("chongwuJiyang/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#chongwuJiyangName").val(ruleForm.chongwuJiyangName);
        $("#chongwuJiyangAge").val(ruleForm.chongwuJiyangAge);
        $("#chongwuJiyangHuase").val(ruleForm.chongwuJiyangHuase);
        $("#chongwuJiyangSuozaidi").val(ruleForm.chongwuJiyangSuozaidi);
        $("#chongwuJiyangContent").val(ruleForm.chongwuJiyangContent);
        $("#yuyueTime-input").val(ruleForm.yuyueTime);
        $("#chongwuJiyangNumber").val(ruleForm.chongwuJiyangNumber);
    ...
    function showImg() {
        <!--  当前表的图片  -->
        $("#chongwuJiyangPhotoImg").attr("src",ruleForm.chongwuJiyangPhoto);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            ...
            initializationChongwujiyangtypesSelect();
            <!--  初始化级联表的下拉框  -->
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".aaaaaa").remove();
            if(window.sessionStorage.getItem('addchongwuJiyang') != null && window.sessionStorage.getItem('addchongwuJiyang') !=""){//新增查询,修改不查
                // 查询当前登录账户的信息
                var userId = window.sessionStorage.getItem('userId');
