    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑宠物领养订单</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑宠物领养订单</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">宠物领养订单信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6 aaaaaa">
                                        <label>宠物</label>
                                        <div>
                                            <select id="chongwuSelect" name="chongwuSelect"
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>试养时间</label>
                                        <input id="shiyangTime-input" name="shiyangTime" type="text" class="form-control layui-input">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#yonghuId") !=null){
            ...
            httpJson("chongwuOrder/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addchongwuOrder');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                chongwuId: "required",
                yonghuId: "required",
                xianzaiAddress: "required",
                shiyangTime: "required",
                insertTime: "required",
            },
            messages: {
                chongwuId: "宠物不能为空",
                yonghuId: "用户不能为空",
                xianzaiAddress: "现在所在地址不能为空",
                shiyangTime: "试养时间不能为空",
                insertTime: "创建时间不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchongwuOrder = window.sessionStorage.getItem("addchongwuOrder");
        if (addchongwuOrder != null && addchongwuOrder != "" && addchongwuOrder != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("chongwuOrder/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#shiyangTime-input").val(ruleForm.shiyangTime);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".aaaaaa").remove();
            if(window.sessionStorage.getItem('addchongwuOrder') != null && window.sessionStorage.getItem('addchongwuOrder') !=""){//新增查询,修改不查
                // 查询当前登录账户的信息
                var userId = window.sessionStorage.getItem('userId');
