...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的宠物领养订单 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.chongwuName}}</td>
                            <td style="width: 80px;">{{item.chongwuValue}}</td>
                            <td style="width: 80px;">{{item.chongwuSexValue}}</td>
                            <td style="width: 60px;">{{item.chongwuAge}}</td>
                            <td style="width: 60px;">{{item.chongwuHuase}}</td>
                            <td style="width: 80px;">{{item.chongwuJiankangValue}}</td>
                            <td style="width: 60px;">{{item.chongwuSuozaidi}}</td>
                            <td style="width: 60px;">{{item.chongwuLingyangyaoqiu}}</td>
                            <td style="width: 60px;">{{item.chongwuLianxifangshi}}</td>
                            <td style="width: 250px;">
                                <img :src="item.chongwuPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.chongwuYesnoValue}}</td>
                            <td style="width: 150px;">{{item.xianzaiAddress}}</td>
                            <td style="width: 150px;">{{item.shiyangTime}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 chongwuOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            chongwuOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(chongwuOrderTypes) {
                this.chongwuOrderTypes = chongwuOrderTypes;
                // 获取列表数据
                layui.http.request('chongwuOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    chongwuOrderTypes: vue.chongwuOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('chongwuOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    chongwuOrderTypes: this.chongwuOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`chongwuOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`chongwuOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('chongwuOrder/list', 'get',{
            page: 1,
            limit: limit,
            chongwuOrderTypes: vue.chongwuOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('chongwuOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            chongwuOrderType: vue.chongwuOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
