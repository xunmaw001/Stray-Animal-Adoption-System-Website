            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">宠物收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="chongwuName" id="chongwuName" placeholder="宠物名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">是否被试养</label>
                              <div class="layui-input-inline">
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../chongwu/detail.jsp?id='+item.chongwuId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.chongwuPhoto?item.chongwuPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../chongwu/detail.jsp?id='+item.chongwuId)"><span>{{item.chongwuName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.chongwuNewMoney"><span style="color: red;">{{item.chongwuNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteChongwuCollection(item.id)">删除 {{item.chongwuName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 chongwuCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            chongwuCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteChongwuCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('chongwuCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#chongwuName').val() != null && jquery('#chongwuName').val() != "" && jquery('#chongwuName').val() != "null" ) {
                param['chongwuName'] = jquery('#chongwuName').val();
            }
            ...
            var chongwuYesnoTypes = $("#chongwuYesnoTypes").find("option:selected").val();
            if (chongwuYesnoTypes != null && chongwuYesnoTypes != "" && chongwuYesnoTypes != "null") {
                            param['chongwuYesnoTypes'] = chongwuYesnoTypes;
                        }
                                        
            // 当前表
                                                    // 获取列表数据
            http.request('chongwuCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function chongwuYesnoTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chongwu_yesno_types", 'get', {}, function (res) {
               if(res.code == 0){
