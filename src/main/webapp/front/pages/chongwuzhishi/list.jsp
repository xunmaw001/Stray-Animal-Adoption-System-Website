            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">宠物知识科普展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">宠物知识科普名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="chongwuzhishiName" id="chongwuzhishiName" placeholder="宠物知识科普名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">宠物科普知识类型</label>
                          <div class="layui-input-inline">
                              <select name="chongwuzhishiTypes" id="chongwuzhishiTypes" lay-filter="chongwuzhishiTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in chongwuzhishiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../chongwuzhishi/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.chongwuzhishiPhoto" />
                                        <h5>{{item.chongwuzhishiName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.chongwuzhishiContent|chongwuzhishiContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            chongwuzhishiTypesList : [],
            chongwuzhishiTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 chongwuzhishiContent 字段去除无用字符 如果字段长度大于60  就截取到60
            chongwuzhishiContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#chongwuzhishiName').val() != null && jquery('#chongwuzhishiName').val() != "" && jquery('#chongwuzhishiName').val() != "null" ) {
                param['chongwuzhishiName'] = jquery('#chongwuzhishiName').val();
            }
         
            var chongwuzhishiTypes = $("#chongwuzhishiTypes").find("option:selected").val();
            if (chongwuzhishiTypes != null && chongwuzhishiTypes != "" && chongwuzhishiTypes != "null") {
                param['chongwuzhishiTypes'] = chongwuzhishiTypes;
            }
                                            // 获取列表数据
            http.request('chongwuzhishi/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function chongwuzhishiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chongwuzhishi_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.chongwuzhishiTypesList = res.data.list;
               }
           });
       }
